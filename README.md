...
Вариант 6. Объявление вещественной константы с
инициализацией на языке C/C++ <br>
Задание: Напишите const double PI = 3.1415; и используйте в
арифметике. Проанализируйте поведение с -constprop или -O2.
Исходный код:
![image](https://github.com/user-attachments/assets/002fd1b6-1697-49da-9687-8faadd5b175c)
Генерация LLVM IR:
Команда: clang++ -S -emit-llvm pi.cpp -o pi.ll
![image](https://github.com/user-attachments/assets/4fd7f9c2-24a0-495a-9a06-95d0729e1b49)
## Оптимизация
Команда opt -O2 -S pi.ll -o pi_opt.ll
![image](https://github.com/user-attachments/assets/eedbac4f-6091-45bd-afcd-fd396e8df619)
## Анализ
Использование const int LIMIT = 100; вместо макроса или обычной переменной позволяет компилятору выполнить подстановку значения напрямую в код. При оптимизации -O2 Clang/LLVM действительно заменяет все обращения к LIMIT на литерал 100, что можно проверить, изучив оптимизированный IR-код (файл .ll). Это не только улучшает производительность, исключая лишние обращения к памяти, но и делает код более безопасным и читаемым по сравнению с макросами. Кроме того, такая константа остаётся частью системы типов C++, что помогает выявлять ошибки на этапе компиляции.
